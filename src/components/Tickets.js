	...
	const handleEditPhoneNumber = async () => {
		...
		try {
			const response = await fetch(
				"http://localhost:8000/api/tickets/editphone",
				{
					method: "POST",
					headers: {
						...
						{data.length > 0 ? (
							data.map((row, idx) => (
								<TableRow key={`${row.id}-${idx}`}>
									...
									<TableCell>
										{ticketTypeComponent(
											row.ticket_jenis === "VIP" ? "Gold" : row.ticket_jenis
										)}
									</TableCell>
									<TableCell>{row.invoice_code}</TableCell>
									<TableCell>{formatDate(row.createdAt)}</TableCell>
									<TableCell>{statusCheckIn(row.check_in_status)}</TableCell>
