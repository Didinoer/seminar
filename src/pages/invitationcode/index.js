...
import { getAllApproval,getCopywriting,getAllInvitationCode } from "../../util/api";
...
export default function VoucherPage(props) {
  ...
  const [loadedVoucher, setloadedVoucher] = useState();
  ...
  const fetchAllVoucher = async () => {
    const data = await getAllInvitationCode();
  ...
  return (
    <MainLayout>
        ...
        <Title>List Invitation Code</Title>
        {isLoading ? (
