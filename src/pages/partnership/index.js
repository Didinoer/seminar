...
import { getAllPartnerships } from "../../util/api";
...
export default function ApprovePage(props) {
  ...
  const [loadedPartnerships, setLoadedPartnerships] = useState();
  ...
  const fetchAllPartnerships = async () => {
    ...
    const data = await getAllPartnerships(params);
    return data;
  };
  ...
  function fetchData() {
    ...
    fetchAllPartnerships()
      .then((result) => {
        console.log(result);
        setLoadedPartnerships(result.partnerships);
        setCountDisplayedData(result.partnerships.length);
    ...
    <MainLayout>
      <Box sx={{ display: "flex", flexWrap: "wrap" }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={3} md={1}>
              <ExportToExcel apiData={loadedPartnerships} fileName={'Export-Partnership'} />
          </Grid>
        </Grid>
        {isLoading ? (
